condor.LevelManager = {
    ...
    tick: function () {
    	...
    	for(var i in enemies) {
			...
			if (enemy.spawned >= enemy.multiple || !enemy.multiple && enemy.spawned) continue;
			
			// not time for this badboy yet
			var delay = enemy.multiple && enemy.spawned ? enemy.delay*enemy.spawned : 0;
			if (time_now-this.time_start < spawn[2]*1000+delay*1000) continue;
			...
			enemy.spawned = _.isNumber(enemy.spawned) ? ++enemy.spawned : 0;
			console.log('spawn')
		}
    }
}